...
def query_download_action(conn, data):
...
def verify_size_action(conn, data):
    file_size = data[0]
...
def process_action(conn, indicator, data):
    if indicator == "query":
        query_action(conn, data)
    elif indicator == "query download":
        query_download_action(conn, data)
    elif indicator == "insert":
        insert_action(conn, data)
    elif indicator == "upload":
        upload_action(conn, data)
    elif indicator == "download":
        download_action(conn, data)
    elif indicator == "verify size":
        verify_size_action(conn, data)
    else:
        print("else")
